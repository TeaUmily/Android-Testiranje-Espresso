...
abstract class ScreenRobot<T : ScreenRobot<T>> {
    ...
    fun checkIsDisabled(@IdRes viewId: Int) : T {
        Espresso.onView(ViewMatchers.withId(viewId)).check(ViewAssertions.matches(Matchers.allOf(
                ViewMatchers.isDisplayed(), Matchers.not(ViewMatchers.isEnabled()))))
        return this as T
    }

    fun checkIsEnabled(@IdRes viewId: Int) : T {
        Espresso.onView(ViewMatchers.withId(viewId)).check(ViewAssertions.matches(Matchers.allOf(
